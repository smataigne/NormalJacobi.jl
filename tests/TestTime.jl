...
l4 = zeros(K)
l5 = zeros(K)
αs = [0.4 0.4]#[0.0 0.0; 0.2 0.2; 0.4 0.4] # You can add more (α, β) pairs if needed
t = size(αs, 1)
N = 5  # Number of trials per case
for k ∈ 1:t
    α₁ = αs[k, 1]
    α₂ = αs[k, 2]
    ...
    plot!(ns, l1, color = :blue, linestyle =:solid, markershape=:circle, label = "Algorithm 5.1")
    ...
    plot!(ns[2:end], ((ns[2:end] ./ 32).^3) .* minimum(l1), color = :gray, linestyle =:dash, label = L"$\mathcal{O}(n^3)$")
    ...
    plot!(ns, l1 ./ l1, color = :blue, linestyle =:solid, markershape=:circle, label = "Algorithm 5.1")
    ...
    ylims!(0.0 , 20)#1.2*maximum(vcat(l2 ./ l1, l3 ./ l1, l4 ./ l1, l5 ./ l1) ) * 1.05)
    title!("Parameters: " * L" $\alpha_1 = $" * string(Int(100α₁))*"%, " * L"$\alpha_2 = $" * string(Int(100α₂)) *"%.")
...
for k ∈ 1:1
    ...
    xlabel!("Matrix size" * L" $n$")   
    ylabel!("Relative running time")
    ylims!(0.0 , 20)# 1.5*maximum(vcat(l2 ./ l1, l3 ./ l1, l4 ./ l1, l5 ./ l1) ) * 1.05)
    title!("Parameters: " * L" $\alpha_1 = $" * string(Int(100α₁))*"%, " * L"$\alpha_2 = $" * string(Int(100α₂)) *"%.")
    savefig(P, "./figures/time_normalskewjacobi_" * string(α₁) * "_" * string(α₂) * "_relative.pdf")
    display(P)
end
