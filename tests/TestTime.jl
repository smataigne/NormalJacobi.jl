...
l4 = zeros(K)
l5 = zeros(K)
αs = [0.2 0.2] # You can add more (α, β) pairs if needed
t = size(αs, 1)
N = 2  # Number of trials per case
for k ∈ 1:t
    α₁ = αs[k, 1]
    α₂ = αs[k, 2]
    ...
    plot!(ns, l1, color = :blue, linestyle =:solid, markershape=:circle, label = "Algorithm 5.3")
