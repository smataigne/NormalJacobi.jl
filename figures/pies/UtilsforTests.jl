...
The parameters α₁ and α₂ determine the fractions of real and repeated complex eigenvalues, respectively
"""
function create_matrix2(n::Integer, α₁::Number, α₂::Number, α₃::Number)
    Q = Matrix(qr(randn(n, n)).Q)
    d = zeros(n)
    sd = zeros(n - 1) 
    a₁ = floor(Int, α₁ * n)
    ...
    d[1:a₁] = 1 .+ rand(a₁)  #real eigenvalues in [1, 2]
    ds = 2 .+ rand(a₂ ÷ 2)
    d[(a₁+1):(a₁+a₂)] = invpermute!([ds; ds], [1:2:a₂;2:2:a₂])
    sd[(a₁+1):2:(a₁+a₂ - 1)] .=  3 + rand()

    s = randn(a₃ ÷ 2)
    d[(a₁ + a₂ + 1):(a₁ + a₂ + a₃)] = invpermute!([s; s], [1:2:a₃;2:2:a₃])
    sd[(a₁ + a₂ + 1):2:(a₁ + a₂ + a₃ - 1)] .= 2 .+  0.01 *√(eps(Float64))  * randn(a₃÷2)
    p = n - a₁ - a₂ - a₃
    s = randn(p ÷ 2)
    d[(a₁ + a₂ + a₃ + 1):end] = invpermute!([s; s], [1:2:p;2:2:p])
    sd[(a₁ + a₂ + a₃ + 1):2:end] .= 1 .+  100 *√(eps(Float64))  * randn(p÷2)
    return Matrix(Q * Tridiagonal(sd, d, -sd) * Q')
end
